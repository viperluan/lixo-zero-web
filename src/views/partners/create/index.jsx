...
const PartnersCreateContainer = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (!nomePatrocionador)
            return toast.warn("Informe o nome do patrocionador!");

        if (!whatsapp)
            return toast.warn("Informe o número de contato!");

        if (!descricaoPatrocinador)
            return toast.warn("Informe a descrição do patrocionador!");

        if (!cotaPatrocinio)
            return toast.warn("Selecione a cota de patrocínio!");
        ...
        setIsLoading(true)
        await api.post(`/patrocinio`, payload).then(res => {
            if (res.data.id) {
                toast.success("Seu formulário foi enviado com sucesso. Em breve a nossa equipe entrará em contato.", {
                    autoClose: 10000
                })
                navigate("/")
            }
        ...
        <Fragment>
            ...
            <Card>
                ...
                <CardBody>
                    <Form className="form">
                        <FormGroup>
                            ...
                            <span className="form-text text-muted small">Empresa/Instituição/Grupo que irá fornecer o patrocínio.
                                Se for 'pessoa física' insira seu nome</span>
                            <Input
                ...
                <CardFooter className="d-flex justify-content-center">
                    <Button onClick={() => handleSubmit()} color="primary">Enviar Formulário</Button>
                </CardFooter>
            </Card>
        </Fragment>
    )
